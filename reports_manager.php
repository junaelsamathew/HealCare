...
if ($role == 'staff') {
    ...
    if (!$staff_type) {
        $check_nurse = $conn->query("SELECT nurse_id FROM nurses WHERE user_id = $user_id");
        if ($check_nurse && $check_nurse->num_rows > 0) $staff_type = 'nurse';
        
        if (!$staff_type) {
            $check_lab = $conn->query("SELECT labstaff_id FROM lab_staff WHERE user_id = $user_id");
            if ($check_lab && $check_lab->num_rows > 0) $staff_type = 'lab_staff';
        }
        
        if (!$staff_type) {
            $check_pharm = $conn->query("SELECT pharmacist_id FROM pharmacists WHERE user_id = $user_id");
            if ($check_pharm && $check_pharm->num_rows > 0) $staff_type = 'pharmacist';
        }
        
        if (!$staff_type) {
            $check_reception = $conn->query("SELECT receptionist_id FROM receptionists WHERE user_id = $user_id");
            if ($check_reception && $check_reception->num_rows > 0) $staff_type = 'receptionist';
        }
        
        if (!$staff_type) {
            $check_canteen = $conn->query("SELECT canteenstaff_id FROM canteen_staff WHERE user_id = $user_id");
...
// Fetch patients for association (only for relevant roles)
$associatable_patients = [];
if (in_array($effective_role, ['admin', 'doctor', 'lab_staff', 'receptionist', 'nurse'])) {
    $p_res = $conn->query("SELECT u.user_id, r.name, pp.patient_code FROM users u JOIN registrations r ON u.registration_id = r.registration_id LEFT JOIN patient_profiles pp ON u.user_id = pp.user_id WHERE u.role = 'patient' ORDER BY r.name ASC");
    while($p_row = $p_res->fetch_assoc()) {
        $associatable_patients[] = $p_row;
    }
}
?>
<!DOCTYPE html>
...
<body>
                ...
                <?php else: ?>
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:30px;">
                        ...
                        <div style="display:flex; gap:10px;">
                            <button class="btn btn-primary" onclick="openUploadModal()"><i class="fas fa-upload"></i> Upload PDF</button>
                    ...
                    <div class="card">
                        ...
                        switch($type) {
                            ...
                            case 'lab_revenue':
                                 ...
                                 break;
                            case 'lab_daily':
                                 $headers = ['Date', 'Test Name', 'Patient', 'Doctor', 'Status'];
                                 $query = "SELECT DATE(l.created_at), l.test_name, rp.name, rd.name, l.status 
                                           FROM lab_tests l 
                                           JOIN users up ON l.patient_id = up.user_id JOIN registrations rp ON up.registration_id = rp.registration_id
                        ...
                        <table id="manualReportTable" style="width: 100%; border-collapse: collapse;">
                            <thead>
                                <tr style="border-bottom: 1px solid rgba(255,255,255,0.1);">
                                    ...
                                    <th style="padding: 12px; text-align: left; color: #94a3b8; cursor: pointer;">Patient <i class="fas fa-sort" style="font-size:10px;"></i></th>
                            ...
                            <tbody>
                                ...
                                <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                                    ...
                                    <td style="padding: 12px; color: #4fc3f7;"><?php 
                                        if($doc['patient_id']) {
                                            $p_id_val = $doc['patient_id'];
                                            $p_check = $conn->query("SELECT r.name FROM users u JOIN registrations r ON u.registration_id = r.registration_id WHERE u.user_id = $p_id_val");
                                            $p_name = ($p_check && $p_row = $p_check->fetch_assoc()) ? $p_row['name'] : 'Unknown';
                                            echo htmlspecialchars($p_name);
                                        } else {
                                            echo '<span style="color:#64748b; font-style:italic;">N/A</span>';
                                        }
                                    ?></td>
                    ...
                    <table id="repoTable">
                        <thead>
                            <tr>
                                ...
                                <th style="cursor:pointer;">Patient <i class="fas fa-sort"></i></th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="color: #4fc3f7;"><?php 
                                    if($doc['patient_id']) {
                                        $p_id_val = $doc['patient_id'];
                                        $p_check = $conn->query("SELECT r.name FROM users u JOIN registrations r ON u.registration_id = r.registration_id WHERE u.user_id = $p_id_val");
                                        echo htmlspecialchars($p_check->fetch_assoc()['name'] ?? 'Unknown');
                                    } else {
                                        echo '-';
                                    }
                                ?></td>
    ...
    <div id="uploadModal" class="upload-modal">
        <div class="modal-content">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:30px;">
                <h3 style="font-size: 20px;">Upload Manual Report</h3>
                <button onclick="closeUploadModal()" style="background:none; border:none; color:var(--text-dim); cursor:pointer; font-size:20px;"><i class="fas fa-times"></i></button>
            </div>
            <form action="upload_report_handler_v2.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="report_type" value="<?php echo htmlspecialchars($type); ?>">
                <input type="hidden" name="redirect_url" value="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>">
                
                <div class="filter-group" style="margin-bottom:20px;">
                    <label>Report Title</label>
                    <input type="text" name="report_title" required placeholder="e.g. <?php echo ucwords(str_replace('_', ' ', $type)); ?> - <?php echo date('M Y'); ?>" style="width:100%;">
                </div>

                <div class="filter-group" style="margin-bottom:20px;">
                    <label>Associate with Patient (Optional)</label>
                    <select name="patient_id" style="width:100%;">
                        <option value="">-- No Specific Patient --</option>
                        <?php foreach($associatable_patients as $p): ?>
                            <option value="<?php echo $p['user_id']; ?>">
                                <?php echo htmlspecialchars($p['name']) . ($p['patient_code'] ? ' ('.$p['patient_code'].')' : ''); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="filter-group" style="margin-bottom:20px;">
                    <label>Select Report File (PDF only)</label>
                    <div style="border: 2px dashed var(--border); padding: 30px; border-radius: 12px; text-align: center; cursor: pointer;" onclick="document.getElementById('reportFile').click()">
                        <i class="fas fa-cloud-upload-alt" style="font-size: 30px; color: var(--primary); margin-bottom: 15px;"></i>
                        <p id="fileName" style="font-size: 14px; color: var(--text-dim);">Click to select or drag & drop</p>
                        <input type="file" name="report_file" id="reportFile" accept=".pdf" required style="display:none;" onchange="updateFileName(this)">
                    </div>
                </div>

                <div class="filter-group" style="margin-bottom:30px;">
                    <label>Report Date</label>
                    <input type="date" name="report_date" value="<?php echo date('Y-m-d'); ?>" style="width:100%;">
                </div>
    ...
    <script>
        ...
        function updateFileName(input) {
            const fileName = input.files[0] ? input.files[0].name : "Click to select or drag & drop";
