...
// Fetch Order Details
$sql = "SELECT o.*, m.item_name, m.image_url, u.username as patient_username, p.name as patient_name, u.email, p.phone
        FROM canteen_orders o
        JOIN canteen_menu m ON o.menu_id = m.menu_id
        LEFT JOIN users u ON o.patient_id = u.user_id 
        LEFT JOIN patient_profiles p ON u.user_id = p.user_id
        WHERE o.order_id = ? AND o.patient_id = ?";
...
if ($order['order_status'] == 'Placed' || $order['order_status'] == 'Paid' || $order['order_status'] == 'Delivered') {
    header("Location: canteen.php?msg=Order+already+paid!");
    exit;
}

$invoice_id = "FOOD-" . str_pad($order['order_id'], 4, '0', STR_PAD_LEFT);
$patient_display = $order['patient_name'] ?? $order['patient_username'] ?? 'Guest';
$service = $order['quantity'] . "x " . $order['item_name'];
...
<head>
    ...
    <title>Food Payment - HealCare PAY</title>
...
<body>
    <div class="processing-overlay" id="loader">
        <i class="fas fa-circle-notch fa-spin fa-3x" style="color:#ea580c;"></i>
    ...
    <div class="main-card">
        <div class="header">
            <div class="logo-area">
                <i class="fas fa-utensils" style="font-size:1.4rem;"></i>
                <span class="logo-txt">HealCare CAFE</span>
            </div>
            <div class="subtitle">Secure Food Payment</div>
        </div>
        ...
        <div class="bill-details">
            ...
            <div class="row">
                <span>Location</span>
                <span class="val"><?php echo htmlspecialchars($order['delivery_location']); ?></span>
            </div>
            <div class="row">
                <span>Order ID</span>
        ...
        <div class="payment-options">
            ...
            <!-- helper form -->
            <form id="verifyForm" action="verify_canteen_payment.php" method="POST" style="display:none;">
            ...
            <div class="secondary-actions">
                <a href="canteen.php" class="sec-btn">
                    <i class="fas fa-times" style="color:#ef4444;"></i> Cancel Order
                </a>
            </div>
            ...
            <!-- Dev Debug Link -->
            <a href="test_canteen_payment_success.php?order_id=<?php echo $order_id; ?>" class="test-mode-link">
    ...
    <script>
        document.getElementById('pay-online-btn').onclick = function(e){
            ...
            var userPhone = "<?php echo $order['phone'] ?? '9999999999'; ?>";
            var userEmail = "<?php echo $order['email'] ?? 'test@example.com'; ?>";

            // 1. Create Order
            fetch('create_canteen_order.php', {
                method: 'POST',
