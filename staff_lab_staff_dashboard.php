...
$cat_q = $conn->query("SELECT category_id FROM lab_categories WHERE category_name = '" . $conn->real_escape_string($lab_type) . "' LIMIT 1");
$category_info = $cat_q->fetch_assoc();
...
<head>
    ...
    <style>
        ...
        .live-validated.invalid { border-color: #ef4444 !important; box-shadow: 0 0 0 2px rgba(239, 68, 68, 0.1); }
        .live-validated.valid { border-color: #10b981 !important; box-shadow: 0 0 0 2px rgba(16, 185, 129, 0.1); }
    </style>
</head>
<body>
    ...
    <div class="dashboard-body">
        ...
        <main class="main-ops">
            <?php if (!isset($_GET['section']) || $_GET['section'] == 'dashboard'): ?>
                ...
                $q_conducted = $conn->query("SELECT COUNT(*) as count FROM lab_tests WHERE category_id = $category_id AND status = 'Conducted'");
                $cond_count = $q_conducted->fetch_assoc()['count'];

                $q_completed = $conn->query("SELECT COUNT(*) as count FROM lab_tests WHERE category_id = $category_id AND status = 'Completed' AND DATE(updated_at) = CURDATE()");
            ...
            <?php elseif ($_GET['section'] == 'conducted'): ?>
                ...
                <div class="test-request-card">
                    <div>
                        ...
                        <div style="background: rgba(245, 158, 11, 0.1); padding: 15px; border-radius: 8px; border: 1px solid rgba(245, 158, 11, 0.2); margin-bottom: 10px;">
                        ...
                        <?php
                        $p_id_chk = $order['patient_id'];
                        $t_name_chk = mysqli_real_escape_string($conn, $order['test_name']);
                        $p_name_chk = mysqli_real_escape_string($conn, $order['patient_name']);
                        
                        $check_m = $conn->query("SELECT * FROM manual_reports WHERE (patient_id = $p_id_chk OR report_title LIKE '%$p_name_chk%') AND (report_title LIKE '%$t_name_chk%' OR created_at >= NOW() - INTERVAL 12 HOUR) ORDER BY created_at DESC LIMIT 1");
                        if ($check_m && $m = $check_m->fetch_assoc()):
                        ?>
                            <div style="background: rgba(16, 185, 129, 0.1); padding: 12px; border-radius: 8px; border: 1px solid rgba(16, 185, 129, 0.3); margin-top: 10px;">
                                <p style="color: #10b981; font-size: 11px; margin: 0;"><i class="fas fa-info-circle"></i> Existing Upload Found: <strong><?php echo htmlspecialchars($m['report_title']); ?></strong></p>
                                <a href="<?php echo htmlspecialchars($m['file_path']); ?>" target="_blank" style="color: #4fc3f7; font-size: 10px; text-decoration: underline;">View existing PDF</a>
                            </div>
                        <?php endif; ?>
                    </div>
                    <div style="background: rgba(255,255,255,0.02); padding: 25px; border-radius: 16px; border: 1px solid var(--border-soft);">
                        <form action="finalize_lab_report.php" method="POST" enctype="multipart/form-data">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 15px;">
                                ...
                                <textarea name="result_summary" class="live-validated result-summary-field" placeholder="Enter findings, observations, and values (or attach PDF below)..." style="width: 100%; background: #020617; border: 1px solid var(--border-soft); padding: 12px; border-radius: 10px; color: #fff; font-size: 13px; resize: none; height: 100px; margin-bottom: 5px;"></textarea>
                                <div class="validation-msg field-msg"></div>
            ...
            <?php elseif ($_GET['section'] == 'completed'): ?>
                ...
                <div style="margin-top: 50px; margin-bottom: 20px;">
                    <h3 style="color:#fff; font-size: 20px;">Manual Document Uploads</h3>
                    <p style="color:#64748b; font-size:12px;">Documents uploaded via Management Console.</p>
                </div>
                <?php
                $sql_manual = "
                    SELECT m.*, r.name as patient_name 
                ...
                ?>
                <div style="background: rgba(15, 23, 42, 0.5); border-radius: 12px; border: 1px dotted var(--border-soft); overflow: hidden;">
                    <table style="width: 100%; border-collapse: collapse; color: #fff;">
                        <thead>
                            <tr style="background: rgba(255,255,255,0.01); text-align: left;">
                                <th style="padding: 15px; font-size: 11px; color: #64748b;">Uploaded On</th>
                                <th style="padding: 15px; font-size: 11px; color: #64748b;">Report Title</th>
                                <th style="padding: 15px; font-size: 11px; color: #64748b;">Patient Associated</th>
                                <th style="padding: 15px; font-size: 11px; color: #64748b;">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php while($m = $res_manual->fetch_assoc()): ?>
                            <tr style="border-top: 1px solid rgba(255,255,255,0.05);">
                                <td style="padding: 15px; font-size: 12px; color: #94a3b8;"><?php echo date('d M, Y', strtotime($m['created_at'])); ?></td>
                                <td style="padding: 15px; font-size: 13px; font-weight: 600;"><?php echo htmlspecialchars($m['report_title']); ?></td>
                                <td style="padding: 15px; font-size: 13px; color: #4fc3f7;"><?php echo htmlspecialchars($m['patient_name'] ?? 'General/None'); ?></td>
                                <td style="padding: 15px;">
                                    <a href="<?php echo htmlspecialchars($m['file_path']); ?>" target="_blank" style="color: #4fc3f7; text-decoration: none; font-size: 11px; border: 1px solid rgba(79, 195, 247, 0.2); padding: 4px 8px; border-radius: 4px;">View File</a>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                        </tbody>
                ...
                <?php else: ?>
                    <div style="text-align: center; padding: 30px; border: 1px dashed var(--border-soft); border-radius: 12px; color: #64748b; font-size: 13px;">No manual uploads found in your history.</div>
                <?php endif; ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const summaries = document.querySelectorAll('.result-summary-field');
            summaries.forEach(el => {
                ...
                el.addEventListener('input', function() {
                    ...
                    if (val.length < 10) {
                        ...
                        msgEl.innerText = "Summary is too short (min 10 chars).";
                        msgEl.classList.add('error');
                    } else if (isGibberish(val)) {
                        el.classList.remove('valid');
                        el.classList.add('invalid');
                        msgEl.innerText = "Summary contains invalid text.";
                        msgEl.classList.add('error');
                    } else {
