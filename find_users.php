...
foreach($users as $name) {
    echo "--- Search: $name ---\n";
    $stmt = $conn->prepare("SELECT u.user_id, u.username, u.role as user_role, r.name, r.registration_id FROM users u JOIN registrations r ON u.registration_id = r.registration_id WHERE r.name LIKE ?");
    $search = "%$name%";
