...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['report_file'])) {
    ...
    if (move_uploaded_file($_FILES["report_file"]["tmp_name"], $target_file)) {
        ...
        $stmt = $conn->prepare("INSERT INTO manual_reports 
            (user_id, patient_id, user_role, report_type, report_category, department, report_title, file_path, report_date, file_size, status) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Submitted')");
        $stmt->bind_param("iisssssssi", $user_id, $patient_id, $role, $report_type, $report_category, $department, $title, $target_file, $report_date, $file_size);
