...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action == 'approve') {
        ...
        // If manually setting to Available, cancel any active leave for today so it doesn't override
        if ($availability == 'Available') {
            $conn->query("UPDATE doctor_leaves SET status = 'Cancelled' WHERE doctor_id = $doc_id AND status = 'Approved' AND CURDATE() BETWEEN start_date AND end_date");
        }
...
try {
    $res_sn = $conn->query("SELECT * FROM notifications WHERE (user_id = 0 OR user_id = ".(isset($_SESSION['user_id'])?$_SESSION['user_id']:0).") AND unread = 1 ORDER BY created_at DESC LIMIT 10");
    if ($res_sn) {
        while($row = $res_sn->fetch_assoc()) {
...
<body>
    ...
    <header class="brand-header">
        <a href="admin_dashboard.php" class="brand-logo-section">
            <img src="images/healcare_logo.jpg" alt="HealCare Logo" class="brand-logo-img">
            <span class="brand-name">HEALCARE HOSPITAL</span>
        </a>
        <div class="brand-info-group">
            <div class="brand-info-item">
                ...
                <div class="brand-info-text">
                    <span class="brand-info-label">WHATSAPP</span>
                    <span class="brand-info-value">+91 953 904 5609</span>
                </div>
            </div>
            <div class="brand-info-item">
                <div class="brand-info-icon">
                ...
                <div class="brand-info-text">
                    <span class="brand-info-label">LOCATION</span>
                    <span class="brand-info-value">Kanjirapally, Kottayam</span>
                </div>
            </div>
        </div>
    </header>
    ...
    <aside class="sidebar">
        ...
        <div class="nav-section">
            ...
            <a href="admin_pharmacy_inventory.php" class="nav-link">
                <i class="fas fa-pills"></i> Medicine Inventory Alert System
            </a>
    ...
    <main class="main-content">
        ...
        <?php if ($section == 'dashboard'): ?>
                ...
                <div style="position: relative;">
                    ...
                    <div class="notification-dropdown" id="notifDropdown">
                        ...
                        <div style="max-height: 400px; overflow-y: auto;">
                            <?php if (empty($system_notifications) && $notif_count == 0 && $pharmacy_alerts == 0 && $expiry_alerts == 0): ?>
                                <div style="padding: 20px; text-align: center; color: var(--text-gray);">
                                    <i class="fas fa-bell-slash" style="font-size: 24px; margin-bottom: 10px; display: block; opacity: 0.5;"></i>
                                    <p style="font-size: 13px;">No new notifications</p>
                                </div>
                            <?php endif; ?>

                            <?php foreach($system_notifications as $sn): ?>
                                <a href="<?php echo $sn['url'] ?? '#'; ?>" class="notification-item" style="border-left: 3px solid var(--accent-<?php echo $sn['color'] ?? 'blue'; ?>);">
                                    <i class="fas <?php echo $sn['icon'] ?? 'fa-info-circle'; ?>" style="color: var(--accent-<?php echo $sn['color'] ?? 'blue'; ?>);"></i>
                                    <div>
                                        <p><?php echo htmlspecialchars($sn['title'] ?? 'System Alert'); ?></p>
                                        <span><?php echo htmlspecialchars($sn['message']); ?></span>
                                    </div>
                                </a>
                            <?php endforeach; ?>
                            ...
                            <?php if ($pharmacy_alerts > 0): ?>
                                <a href="admin_pharmacy_inventory.php?section=low_stock" class="notification-item" style="align-items: flex-start;">
                            ...
                            <?php if ($expiry_alerts > 0): ?>
                                <a href="admin_pharmacy_inventory.php?section=expiry" class="notification-item" style="align-items: flex-start;">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card" style="cursor: pointer;" onclick="window.location.href='admin_pharmacy_inventory.php'">
                    <div class="stat-card-header">
                    ...
                    <div class="stat-card-value" style="color: var(--accent-red);"><?php echo ($pharmacy_alerts + $expiry_alerts); ?></div>
                    <div class="stat-card-subtitle"><?php echo $pharmacy_alerts; ?> Low stock • <?php echo $expiry_alerts; ?> Expiring</div>
                </div>
            </div>
        ...
        <?php elseif ($section == 'room-management'): ?>
            ...
            <div class="content-section" style="margin-bottom: 30px; border: 1px solid rgba(245, 158, 11, 0.3); background: rgba(245, 158, 11, 0.05);">
                ...
                <div class="filter-container" style="margin-bottom: 20px;">
                    <div class="search-input-group">
                        ...
                        <input type="text" id="pendingAdmSearch" placeholder="Search patients or doctors..." onkeyup="filterTable('pendingAdmSearch', 'pendingAdmissionsTable')">
                    </div>
                </div>

                <table id="pendingAdmissionsTable">
                    <thead>
                        <tr>
                            <th>Patient</th>
                            <th>Doctor</th>
            ...
            <div class="content-section">
                ...
                <div class="filter-container" style="margin-bottom: 20px;">
                    <div class="search-input-group">
                        <i class="fas fa-search"></i>
                        <input type="text" id="wardSearch" placeholder="Search ward name or type..." onkeyup="filterTable('wardSearch', 'wardsTable')">
                    </div>
                </div>
            ...
            <div class="content-section">
                ...
                <div class="filter-container" style="margin-bottom: 20px; flex-wrap: wrap;">
                    <div class="search-input-group" style="min-width: 250px;">
                        <i class="fas fa-search"></i>
                        <input type="text" id="roomSearch" placeholder="Search room number, ward or patient..." onkeyup="advancedRoomFilter()">
                    </div>
                    <select id="wardTypeFilter" class="filter-select" onchange="advancedRoomFilter()">
                    ...
                    <select id="roomStatusFilter" class="filter-select" onchange="advancedRoomFilter()">
                ...
                <table id="roomsTable">
                    ...
                    <tbody>
                        <?php 
                        $rooms_sql = "SELECT r.*, w.ward_name, w.ward_type,
                        ...
                        ?>
                        <tr class="room-row" data-ward-type="<?php echo htmlspecialchars($room['ward_type']); ?>" data-status="<?php echo $status_text; ?>">
                ...
                ?>
                        <div style="background: rgba(255,255,255,0.02); border: 1px solid var(--border-color); border-radius: 16px; padding: 20px; margin-bottom: 15px; display: flex; justify-content: space-between; align-items: center; transition: all 0.3s; hover: background: rgba(255,255,255,0.04);">
                            <div style="display: flex; align-items: center; gap: 20px;">
                                ...
                                <div>
                                    <h4 style="margin: 0; font-size: 16px;"><?php echo htmlspecialchars($doc['name']); ?></h4>
                                    <p style="font-size: 12px; color: var(--text-gray);"><?php echo htmlspecialchars($doc['specialization']); ?> &bull; <?php echo htmlspecialchars($doc['username']); ?></p>
                                    <span class="badge badge-<?php 
        ...
        <?php elseif ($section == 'analytics'): ?>
            ...
            try {
                // Revenue Stats
                $rev_data = $conn->query("SELECT 
                    SUM(total_amount) as total,
                    SUM(CASE WHEN DATE(bill_date) = CURDATE() THEN total_amount ELSE 0 END) as today_rev,
                    COUNT(DISTINCT patient_id) as total_patients
                    FROM billing WHERE payment_status = 'Paid'")->fetch_assoc();
                
                $total_revenue = $rev_data['total'] ?? 0;
                $today_revenue = $rev_data['today_rev'] ?? 0;
                $unique_patients = $rev_data['total_patients'] ?? 0;

                // Appointment Stats
                $appt_stats = $conn->query("SELECT 
                    COUNT(*) as total,
                    SUM(CASE WHEN appointment_date = CURDATE() THEN 1 ELSE 0 END) as today_appts,
                    SUM(CASE WHEN status = 'Completed' THEN 1 ELSE 0 END) as completed
                    FROM appointments")->fetch_assoc();
                
                $total_appointments = $appt_stats['total'] ?? 0;
                $today_appointments = $appt_stats['today_appts'] ?? 0;
                $success_rate = ($total_appointments > 0) ? round(($appt_stats['completed'] / $total_appointments) * 100) : 0;
                ...
                // Fallback
                $total_revenue = $today_revenue = $unique_patients = $total_appointments = $today_appointments = $success_rate = 0;
                $rev_by_cat = [];
            }

            // --- FILTER LOGIC (MANUAL REPORTS) ---
            $f_type = $_GET['f_type'] ?? '';
            ...
            if ($result) {
                while($row = $result->fetch_assoc()) {
                    ...
                    // Populate Analytics (with trimming and sanitization)
                    $t = trim($row['report_type'] ?: 'Unspecified');
                    $dist_type[$t] = ($dist_type[$t] ?? 0) + 1;

                    $c = trim($row['report_category'] ?: 'General');
                    $dist_role[$c] = ($dist_role[$c] ?? 0) + 1;

                    $raw_date = $row['report_date'];
                    $d = ($raw_date && $raw_date != '0000-00-00') ? date('M d', strtotime($raw_date)) : 'N/A';
                    $timeline[$d] = ($timeline[$d] ?? 0) + 1;
                }
            }
            // ksort($timeline); // Timeline is better sorted by date, but displayed as labels
            ?>

            <!-- Quick Intelligence Metrics -->
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 20px; margin-bottom: 30px;">
                <div class="stat-card" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); border: 1px solid rgba(59, 130, 246, 0.2);">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div class="stat-card-title">Total Revenue</div>
                            <div class="stat-card-value" style="color: #10b981;">Rs. <?php echo number_format($total_revenue); ?></div>
                            <div class="stat-card-subtitle"><i class="fas fa-arrow-up"></i> Rs. <?php echo number_format($today_revenue); ?> recorded today</div>
                        </div>
                        <i class="fas fa-money-bill-wave" style="font-size: 30px; color: rgba(16, 185, 129, 0.2);"></i>
                    </div>
                </div>
                <div class="stat-card" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); border: 1px solid rgba(59, 130, 246, 0.2);">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div class="stat-card-title">Patient Flow</div>
                            <div class="stat-card-value" style="color: #3b82f6;"><?php echo number_format($total_appointments); ?></div>
                            <div class="stat-card-subtitle"><?php echo $today_appointments; ?> visits scheduled for today</div>
                        </div>
                        <i class="fas fa-users" style="font-size: 30px; color: rgba(59, 130, 246, 0.2);"></i>
                    </div>
                </div>
                <div class="stat-card" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); border: 1px solid rgba(59, 130, 246, 0.2);">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div class="stat-card-title">Clinical Success</div>
                            <div class="stat-card-value" style="color: #f59e0b;"><?php echo $success_rate; ?>%</div>
                            <div class="stat-card-subtitle">Appointment fulfillment rate</div>
                        </div>
                        <i class="fas fa-check-circle" style="font-size: 30px; color: rgba(245, 158, 11, 0.2);"></i>
                    </div>
                </div>
                <div class="stat-card" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); border: 1px solid rgba(59, 130, 246, 0.2);">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div class="stat-card-title">Archive Volume</div>
                            <div class="stat-card-value" style="color: #8b5cf6;"><?php echo count($reports); ?></div>
                            <div class="stat-card-subtitle">Digitized reports indexed</div>
                        </div>
                        <i class="fas fa-file-medical" style="font-size: 30px; color: rgba(139, 92, 246, 0.2);"></i>
                    </div>
                </div>
            </div>
            ...
            <div class="content-section">
                <div class="section-header">
                    <h3 class="section-title"><i class="fas fa-chart-pie"></i> Intelligence Overview</h3>
                    <div style="font-size: 13px; color: #94a3b8;">Real-time analysis based on operations and database records</div>
                </div>
                
                <!-- Main Operational Charts -->
                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 20px; margin-bottom: 25px;">
                    <div style="background: rgba(15, 23, 42, 0.5); padding: 25px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.05); height: 420px; display: flex; flex-direction: column;">
                        <h4 style="font-size: 14px; color: #94a3b8; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1px; font-weight: 700;">Revenue Distribution by Service</h4>
                        <div style="flex: 1; position: relative; min-height: 0;">
                            <canvas id="chartRevenue"></canvas>
                        </div>
                    </div>
                    <div style="background: rgba(15, 23, 42, 0.5); padding: 25px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.05); height: 420px; display: flex; flex-direction: column;">
                        <h4 style="font-size: 14px; color: #94a3b8; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1px; font-weight: 700;">Report Composition</h4>
                        <div style="flex: 1; position: relative; min-height: 0;">
                ...
                <!-- Secondary Charts Grid -->
                 <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 20px;">
                    <div style="background: rgba(15, 23, 42, 0.5); padding: 25px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.05); height: 350px; display: flex; flex-direction: column;">
                        <h4 style="font-size: 14px; color: #94a3b8; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1px; font-weight: 700;">Departmental Activity (Archival)</h4>
                        <div style="flex: 1; position: relative; min-height: 0;">
                            <canvas id="chartRole"></canvas>
                        </div>
                    </div>
                    <div style="background: rgba(15, 23, 42, 0.5); padding: 25px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.05); height: 350px; display: flex; flex-direction: column;">
                        <h4 style="font-size: 14px; color: #94a3b8; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1px; font-weight: 700;">Submission History</h4>
                        <div style="flex: 1; position: relative; min-height: 0;">
    ...
    <script>
        ...
        function advancedRoomFilter() {
            const search = document.getElementById('roomSearch').value.toLowerCase();
            const typeReq = document.getElementById('wardTypeFilter').value.toLowerCase();
            const statusReq = document.getElementById('roomStatusFilter').value.toLowerCase();
            const rows = document.querySelectorAll('.room-row');

            rows.forEach(row => {
                const text = row.innerText.toLowerCase();
                const wardType = row.getAttribute('data-ward-type').toLowerCase();
                const status = row.getAttribute('data-status').toLowerCase();

                const matchesSearch = text.includes(search);
                const matchesType = typeReq === "" || wardType === typeReq;
                const matchesStatus = statusReq === "" || status === statusReq;
