        ...
        <?php if ($section == 'view-feedback'): ?>
            <div class="top-bar">
                <div class="page-title">
                    ...
                    <p>Review and analyze patient satisfaction and comments.</p>
                </div>
                <div class="filter-controls" style="display: flex; gap: 10px;">
                    <select id="ratingFilter" onchange="filterFeedback()" style="background:var(--darker-blue); color:white; padding:8px 15px; border-radius:8px; border:1px solid var(--border-color);">
            ...
            <div id="feedbackGrid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 25px;">
                <!-- Feedback cards will be injected here via JS -->
            </div>
            
            <?php
            // Fetch Feedback
            $feedback_items = [];
            $fb_sql = "SELECT f.*, p.name as patient_name, u.username 
                       FROM patient_feedback f 
                       JOIN users u ON f.patient_id = u.user_id 
                       LEFT JOIN registrations p ON u.registration_id = p.registration_id
                       ORDER BY f.created_at DESC";
            $fb_res = $conn->query($fb_sql);
            if ($fb_res && $fb_res->num_rows > 0) {
                while($fb = $fb_res->fetch_assoc()) {
                    // Map experience to emoji and type
                    $exp = $fb['experience']; // "Excellent", "Good", "Average", "Poor"
                    $emoji = "😐";
                    $type = "neutral";
                    
                    if (strpos($exp, 'Excellent') !== false) { $emoji = "😊"; $type = "positive"; }
                    elseif (strpos($exp, 'Good') !== false) { $emoji = "🙂"; $type = "positive"; }
                    elseif (strpos($exp, 'Poor') !== false) { $emoji = "🙁"; $type = "negative"; }
                    
                    $feedback_items[] = [
                        'id' => $fb['feedback_id'],
                        'patient' => $fb['patient_name'] ?? $fb['username'],
                        'date' => date('Y-m-d H:i', strtotime($fb['created_at'])),
                        'experience' => strip_tags($fb['experience']),
                        'exp_emoji' => $emoji,
                        'doctor_rating' => strip_tags($fb['doctor_rating']),
                        'cleanliness' => (int)$fb['cleanliness'],
                        'staff_response' => strip_tags($fb['staff_response']),
            ...
            <script>
            // Real Data from PHP
            const feedbackData = <?php echo json_encode($feedback_items); ?>;
            ...
            function renderFeedback(data) {
                ...
                if (data.length === 0) {
                    grid.innerHTML = '<p style="grid-column: 1/-1; text-align: center; color: var(--text-gray); padding: 40px;">No feedback found.</p>';
                    return;
                }

                data.forEach(item => {
                    let borderClass = '';
                    if(item.type === 'positive') borderClass = 'border-left: 4px solid #10b981;';
                    else if(item.type === 'neutral') borderClass = 'border-left: 4px solid #f59e0b;';
                    else if(item.type === 'negative') borderClass = 'border-left: 4px solid #ef4444;';

                    // Generate Stars
                    let stars = '';
                    for(let i=0; i<5; i++) {
                        stars += i < item.cleanliness ? '<i class="fas fa-star" style="color:#fbbf24;"></i>' : '<i class="far fa-star" style="color:#475569;"></i>';
                    }

                    const card = `
                        <div class="feedback-card" id="card-${item.id}" style="background: var(--dark-blue); border-radius: 16px; padding: 25px; border: 1px solid var(--border-color); ${borderClass} box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                            <div style="display:flex; justify-content:space-between; margin-bottom:15px;">
                                <div>
                                    <h4 style="margin:0; color:white; font-size:16px;">${item.patient}</h4>
                                    <small style="color:var(--text-gray); font-size:12px;">${item.date}</small>
                                </div>
                                <form method="POST" onsubmit="return confirm('Delete this feedback?');" style="margin:0;">
                            ...
                            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; margin-bottom:15px; background:rgba(0,0,0,0.2); padding:10px; border-radius:10px;">
                                <div>
                                    <span style="display:block; font-size:10px; color:var(--text-gray); text-transform:uppercase;">Experience</span>
                                    <span style="color:white; font-weight:600;">${item.exp_emoji} ${item.experience}</span>
                                </div>
                                <div>
                                    <span style="display:block; font-size:10px; color:var(--text-gray); text-transform:uppercase;">Cleanliness</span>
                                    <span>${stars}</span>
                                </div>
                                <div>
                                    <span style="display:block; font-size:10px; color:var(--text-gray); text-transform:uppercase;">Doctor Rating</span>
                                    <span style="color:${item.doctor_rating.includes('Yes') || item.doctor_rating.includes('Satisfied') ? '#10b981' : (item.doctor_rating.includes('No') ? '#ef4444' : '#f59e0b')}; font-weight:600;">${item.doctor_rating}</span>
                                </div>
                                <div>
                                    <span style="display:block; font-size:10px; color:var(--text-gray); text-transform:uppercase;">Staff Response</span>
                                    <span style="color:${item.staff_response.includes('Yes') ? '#10b981' : '#ef4444'}; font-weight:600;">${item.staff_response}</span>
                                </div>
                            </div>

                            <div>
                                <span style="display:block; font-size:11px; color:var(--text-gray); margin-bottom:5px; font-weight:600;">PATIENT COMMENTS:</span>
                                <p style="margin:0; font-size:13px; color:#cbd5e1; line-height:1.5; font-style:italic;">"${item.comment}"</p>
                            </div>
                        </div>
                    `;
                    grid.innerHTML += card;
            ...
            function filterFeedback() {
                const type = document.getElementById('ratingFilter').value;
                if(type === 'all') {
                    renderFeedback(feedbackData);
                } else {
                    const filtered = feedbackData.filter(item => item.type === type);
